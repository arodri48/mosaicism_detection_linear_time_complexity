...
class child():
    ...
    def __init__(self, child_name, father_name, mother_name):
        ...
        self.dad_rd_array = []
        self.mom_rd_array = []
        self.dad_ref_var_array = []
        self.mom_ref_var_array = []
        self.child_ref_rd = []
        self.child_var_rd = []
    def phasable_snp_determiner(self, chr_df):
        ...
        for index, row in chr_df.iterrows():
            ...
            if (child_info[0] in self.het_set):
                # proband is a het; need to check the parents and check at least one is homozygous and if they are both homozygous, not for same allele
                ...
                if (not((mom_geno in self.het_set) and (dad_geno in self.het_set)) and not ((mom_geno in self.hom_ref and dad_geno in self.hom_ref) or (mom_geno in self.hom_var and dad_geno in self.hom_var))):
    ...
    def t_test_snps(self, samp_size):
        ...
        if (samp_size > self.child_ref_rd.size):
            ...
            t_values = np.zeros(size=(self.child_ref_rd.size - samp_size + 1))
