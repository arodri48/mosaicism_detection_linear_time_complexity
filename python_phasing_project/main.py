...
def main(config_file_path):
    ...
    with open(config_elem["OUTPUT_FILE"], 'w') as output_file:
        ...
        for i, elem in enumerate(mosaicism_outcome):
            if elem is not None:
                if elem[5] == 0:
                    output_file.write("\t".join([str(i + 1), str(elem[0]), str(elem[1]), str(elem[5]), "N/A", "N/A", str(elem[6]), str(elem[7])]))
                else:
                    # y shaped mosaicism is present
                    output_file.write("\t".join([str(i + 1), str(elem[0]), str(elem[1]), str(elem[5]), str(elem[6]), str(elem[7]), "N/A", "N/A"]))
                output_file.write("\n")
